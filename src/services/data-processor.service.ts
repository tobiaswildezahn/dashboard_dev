/**
 * Data Processor Service - Verarbeitet und berechnet alle Metriken
 */
...
/**
 * Verarbeitet Roh-Daten und fügt berechnete Felder hinzu
 */
export function processData(
  rawResourceFeatures: Feature<ResourceAttributes>[],
  rawEventFeatures: Feature<EventAttributes>[]
): ProcessedEinsatz[] {
  // Event-Map für schnellen Lookup erstellen
  const eventMap = new Map<string, EventAttributes>();
...
/**
 * Berechnet alle KPI-Metriken aus den verarbeiteten Daten
 */
export function calculateKPIs(data: ProcessedEinsatz[]): KPIMetrics {
  const total = data.length;
  const relevantData = data.filter(d => d.isHilfsfristRelevant);
  const notRelevantData = data.filter(d => !d.isHilfsfristRelevant);

  // Response Time KPIs (nur hilfsfristrelevant)
  ...
  // Travel Time KPIs (nur hilfsfristrelevant)
  ...
  // Hilfsfrist KPIs (nur hilfsfristrelevant)
...
/**
 * Aggregiert Daten stündlich für Line Chart
 */
export function aggregateHourlyStats(data: ProcessedEinsatz[]): Map<string, HourlyStats> {
  const relevantData = data.filter(d => d.isHilfsfristRelevant);
...
/**
 * Erstellt Response Time Bins (10 Sekunden Schritte)
 */
export function createResponseTimeBins(data: ProcessedEinsatz[]): ResponseTimeBins {
  const relevantData = data.filter(d => d.isHilfsfristRelevant && d.responseTime !== null);

  const bins: ResponseTimeBins = {
    '0-10s': 0,
    '10-20s': 0,
    '20-30s': 0,
    '30-40s': 0,
...
/**
 * Erstellt Travel Time Bins (1 Minute Schritte)
 */
export function createTravelTimeBins(data: ProcessedEinsatz[]): TravelTimeBins {
  const relevantData = data.filter(d => d.isHilfsfristRelevant && d.travelTime !== null);

  const bins: TravelTimeBins = {
    '0-1min': 0,
    '1-2min': 0,
    '2-3min': 0,
    '3-4min': 0,
 ...
 */
export function filterByRtw(
  data: ProcessedEinsatz[],
...
/**
 * Extrahiert eindeutige RTW-Kennzeichen
 */
export function extractUniqueRtw(data: ProcessedEinsatz[]): string[] {
